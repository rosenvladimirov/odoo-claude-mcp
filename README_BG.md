...
## 📦 Съпътстващи Odoo модули и тема

MCP стекът работи заедно с **няколко Odoo модула и website тема**.
Заедно те превръщат всяка Odoo инстанция в пълноценна MCP-aware,
мулти-тенант, billing-ready AI работна среда — с опционален editorial
website skin отгоре.
...
- **Odoo 18**: [`l10n-bulgaria → l10n_bg_claude_terminal`](https://github.com/rosenvladimirov/l10n-bulgaria/tree/18.0/l10n_bg_claude_terminal) · актуално: **18.0.1.36.1**
- **Odoo 19**: [`l10n-bulgaria → l10n_bg_claude_terminal`](https://github.com/rosenvladimirov/l10n-bulgaria/tree/19.0/l10n_bg_claude_terminal) · актуално: **19.0.1.34.1**
- **Odoo 16**: същото repo, клон `16.0`

> 🌍 **Не-български Odoo инсталации?** Ползвай международния fork
> [`rosenvladimirov/odoo-mcp-terminal`](https://github.com/rosenvladimirov/odoo-mcp-terminal)
> — същият модул, но **без** BG localization зависимостите (без НАП
> полета, без BG transliteration mixin). Клонове `16.0` / `18.0` / `19.0`,
> AGPL-3.0. Препоръчителен за всяка Odoo инстанция извън България.
...
### `odoo-mcp-theme` — Editorial Odoo website тема (безплатна, AGPL-3)

Editorial **"paper" тема** разработена за MCP Works landing страницата
и за всяка Odoo website, която иска чист, content-first дизайн. В
production на [mcpworks.net](https://mcpworks.net).

**Repo:**
[`rosenvladimirov/odoo-mcp-theme`](https://github.com/rosenvladimirov/odoo-mcp-theme)
· клон **19.0** · актуално: **19.0.x**

**Какво носи:**

- **OKLCH paper/ink + indigo** цветова система — perceptually-uniform
  палитра, която се принтира добре, скенира добре, и се адаптира за
  dark mode без загуба на контраст
- **Custom snippets** — pricing карти (5-tier layout), feature grid-ове,
  setup-wizard CTA-та, demo-link блокове, MCP architecture диаграми
- **Vertical timeline language switcher** — заменя default-ния dropdown
  на Odoo с sidebar timeline (работи добре за trilingual EN / BG / RU)
- **Editorial typography** — оптимизирана за long-form постове, ≤ 70
  знака на ред, измерен vertical rhythm
- **Напълно `#wrapwrap`-scoped** — без leak в Odoo backend или Website
  Builder editor (избягва `$o-color-palettes-name` глобалния leak,
  който чупи editor UI на по-стари теми)

Инсталираема на всяка Odoo 19 инстанция. Сложи я в addons path,
инсталирай през Apps, после задай в **Website → Configuration → Themes**.

### Ред на инсталация

```
odoo-mcp-theme            ← опционално: editorial website skin
...
`odoo-mcp-theme` е независима и може да се ползва със или без MCP стека.
...
### Админ конзола (`/admin`) и втори фактор

Скрита уеб конзола вътре в `odoo-rpc-mcp` за хората, които оперират стака:
Odoo връзки на потребител, създаване на потребители с еднократен ключ, backups,
filestore. Изключена е, докато не се конфигурира.

| Env | Значение |
|---|---|
| `MCP_ADMIN_PATH_PREFIX` | Път на монтиране, по подразбиране `/admin`; празно = изключена |
| `MCP_BOOTSTRAP_ADMIN` | Odoo login, който става админ при първото влизане |
| `MCP_SECRET_TOKEN` / `MCP_ADMIN_SESSION_SECRET` | Подписва бисквитките на сесията. **Задължителен** — без него конзолата отказва да се качи |
| `MCP_KEY_PEPPER` | ≥32 знака. Криптира TOTP тайните на диска (същият pepper като `identify_totp_*`) |
| `MCP_ADMIN_REQUIRE_TOTP` | `admins` или `all` — държи потребителите в обхвата на страницата „Сигурност“, докато не запишат втори фактор |
| `MCP_ADMIN_KNOCK_TOKEN`, `MCP_ADMIN_ALLOWED_IPS` | По желание: knock токен (`?k=`) и allowlist по IP/CIDR |

**Втори фактор (3.3.8).** Всеки потребител записва TOTP от „Сигурност“: парола → QR →
потвърждаващ код → осем еднократни кода за възстановяване. След това всеки път за
сесия (MCP парола, Odoo re-auth, еднократен ключ) иска кода. Тайните са в
`/data/users/<login>/totp.json`; админът нулира фактор от „Потребители“, а последният
админ се възстановява, като изтрие този файл на тома. Всичко се пише в
`/data/admin_audit.log` — никога тайната или кодът.
