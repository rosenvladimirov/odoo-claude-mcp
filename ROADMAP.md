...
- **2026-06-03** — Phase 2b DONE: persistent asyncio loop + NewMessage→`centrifugo_client.publish(telegram:<principal>)`; MCP env wired (CENTRIFUGO_API_URL/KEY) + redeployed to 202588745 (clean boot, dormant — no telegram auth there). Unit 4/4.
- **2026-06-03** — Phase 4 transport **E2E PROVEN** over public https://centrifugo.mcpworks.net: JWT mint → `/connection/uni_sse` subscribe (token-only, server-side sub via channels claim) → publish → received. Centrifugo config: `uni_sse` enabled (the wake-loop endpoint); `/connection/sse` is bidirectional-emulation and rejects plain GET (3501). `tools/tg_wakeloop.sh` updated accordingly.
- **Remaining to fully close the loop:** run `tg_wakeloop.sh` under Monitor on Rosen's **authed** production MCP + retire the hourly storytelling cron. Minor: tighten Centrifugo `allowed_origins` from `*`.
