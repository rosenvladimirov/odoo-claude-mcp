...
def test_safe_save_path_blocks_absolute_and_traversal(srv, monkeypatch):
    monkeypatch.setattr(srv, "_require_principal", lambda: "tester")
    base = os.path.realpath(os.path.join(srv._user_dir("tester", create=True), "downloads"))

    # benign relative leaf stays inside
    p = srv._safe_save_path("report.pdf")
    assert p.startswith(base + os.sep)

    # absolute path is treated as relative leaf and confined (not honored as /etc/...)
    p2 = srv._safe_save_path("/etc/cron.d/x")
    assert p2.startswith(base + os.sep)
    assert "/etc/cron.d" not in p2 or p2.startswith(base)
